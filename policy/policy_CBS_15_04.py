...
# A* for CBS fallback and low-level search with constraints
def a_star_constrained(env, agent, start, goal, constraints):
	...
	while open_list != []:
		...
		# Neighbor Expansion, we check all the neighbors of the current node
...
def update_stuck(env):
	...
	for agent in range(env.agent_num):
		...
		if stuck[agent] > STUCK_THRESHOLD: # Path replan
			stuck[agent] = 0 
...
def cbs(env):
	...
	while open_list and iter_count < max_iter:
		...
		low = max(a1, a2)
		# We only branch on the lower-priority agent to reduce branching factor
